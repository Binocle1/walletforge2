    ...
    switch (type) {
      case 'purchase': {
        if (program.type === 'giftcard') {
          ...
          // BUGFIX : la récompense n'était plus jamais débloquée (régression v4).
          const newStamps = pass.stamps + stampsDelta;
          if (program.stamps_required && newStamps >= program.stamps_required) {
            rewardsDelta = Math.floor(newStamps / program.stamps_required);
            stampsDelta = (newStamps % program.stamps_required) - pass.stamps; // on garde le reste
            message = `🎉 ${rewardsDelta > 1 ? rewardsDelta + ' récompenses débloquées' : 'Récompense débloquée'} : ${program.reward_label || 'récompense'} !`;
          } else {
            const left = (program.stamps_required || 0) - newStamps;
            message = left > 0
              ? `Merci ! Plus que ${left} tampon${left > 1 ? 's' : ''} avant votre récompense.`
              : `Merci ! ${stampsDelta} tampon(s) ajouté(s).`;
          }
        } else {
          // Programme points : là, le montant est bien obligatoire.
          if (!Number.isFinite(Number(amount)) || Number(amount) <= 0) {
            throw new Error('Montant requis pour un achat (programme à points)');
          }
