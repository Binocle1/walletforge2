...
function go(s) {
  ...
  const fns = { home: loadHome, programs: loadPrograms, customers: loadCustomers, tx: loadTx, stats: loadStats, notifs: loadNotifs, settings: loadBusiness };
  if (fns[s]) fns[s]();
}
...
async function loadHome() {
  ...
  document.getElementById('statCards').innerHTML = [
    ...
    ['Taux de retour', (s.return_rate != null ? s.return_rate : 0) + ' %'], ['Panier moyen', (s.avg_basket != null ? s.avg_basket : 0) + ' €'],
  ...
  ].map(([l, v]) => `<div class="card stat"><small>${l}</small><h2>${v != null ? v : 0}</h2></div>`).join('');
...
async function loadCustomers() {
  ...
  document.querySelector('#custTable tbody').innerHTML = list.map(c => {
    ...
    return `<tr>
      ...
      <td>${esc(p.program || '—')}</td>
      <td>${p.type === 'stamps' ? (p.stamps != null ? p.stamps : 0) + ' tampons' : Number(p.points != null ? p.points : 0) + ' pts'}${p.rewards > 0 ? ` <span class="pill a">${p.rewards} 🎁</span>` : ''}</td>
